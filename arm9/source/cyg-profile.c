...
void cygprofile_begin(void)
{
	...
	if (openlogfile (cyg_profile_filename) == NULL)
	{
		printf("couldn\'t open perf log file, %s\n", cyg_profile_filename);
	}
...
cygprofile_end(void)
{
	...
	printf("profiling is done, dumping to disk\n");
	...
	for (count = 0; count < perf_position; count++)
	{
		...
		//_ds_fwrite(buf, strlen(buf), 1, logfile, NULL);
